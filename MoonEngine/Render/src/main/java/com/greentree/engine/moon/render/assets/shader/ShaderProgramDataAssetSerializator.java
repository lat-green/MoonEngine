...
public class ShaderProgramDataAssetSerializator implements AssetSerializator<ShaderProgramDataImpl> {
    ...
    @Override
    public AssetProvider<ShaderProgramDataImpl> load(AssetLoader.Context manager, AssetKey ckey) {
...
//        if (geom.isValid()) {
//            return AssetProviderKt.map(vert, frag, geom, new VertFragGeomShader());
//        }
        return AssetProviderKt.map(vert, frag, new VertFragShader());
    }
