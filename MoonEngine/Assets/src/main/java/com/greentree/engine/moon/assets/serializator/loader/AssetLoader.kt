...
inline fun <reified T : Any> AssetLoader.Context.load(key: String): Asset<T> {
	val type = TypeInfoBuilder.getTypeInfo(T::class.java)
	return load(type, key)
}

fun <T : Any> AssetLoader.Context.load(type: TypeInfo<T>, key: Any) = load(type, ResultAssetKey(key))
fun <T : Any> AssetLoader.Context.load(type: TypeInfo<T>, key: String) = load(type, ResourceAssetKey(key))