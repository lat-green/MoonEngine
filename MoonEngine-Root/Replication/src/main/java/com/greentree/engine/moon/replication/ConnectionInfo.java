...
import com.greentree.engine.moon.replication.event.*;
...
public class ConnectionInfo implements AutoCloseable {
    ...
    public final void write(Iterable<? extends Entity> replication) {
    ...
    protected void initReadThream(Consumer<? super Iterable<? extends EntityEvent>> events) {
        ...
        final var getThread = new Thread(() -> {
            try {
                while (running[0]) {
                    final var bs = read();
                    @SuppressWarnings("unchecked") final var evs = (Iterable<? extends EntityEvent>) deser(bs);
                    events.accept(evs);
                }
